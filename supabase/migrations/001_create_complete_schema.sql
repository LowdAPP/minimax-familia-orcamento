...
-- Index para performance
CREATE INDEX idx_budget_envelopes_budget ON budget_envelopes(budget_id);
...
-- Index para performance
CREATE INDEX idx_debt_plans_user_id ON debt_payoff_plans(user_id);
...
-- Index para performance
CREATE INDEX idx_debt_items_plan ON debt_items(plan_id);
CREATE INDEX idx_debt_items_priority ON debt_items(plan_id, priority_order);
...
CREATE TRIGGER update_budget_envelopes_updated_at BEFORE UPDATE ON budget_envelopes
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_debt_items_updated_at BEFORE UPDATE ON debt_items
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ============================================
-- 14. STORAGE BUCKETS
-- Criar buckets no Supabase Dashboard ou via SQL (se disponível)
-- ============================================
-- bucket: bank-statements (para PDFs de extratos bancários)
-- bucket: user-avatars (para fotos de perfil - opcional)
